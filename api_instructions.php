...
$isDefaultKey  = ($apiKey === 'change_me_to_a_random_string');
$apiUrl        = BASE_URL . '/api.php';
$proxyUrl      = BASE_URL . '/api_console_proxy.php';
...
<div class="container">
    ...
    <div class="doc-section">
        ...
        <p><strong>Create:</strong> <code>POST /api.php</code> with JSON body</p>

        <h2>3. PHP Example</h2>
        <pre>
$ch = curl_init('<?= htmlspecialchars($apiUrl) ?>');
...
curl_close($ch);</pre>
        ...
        <p>The API enforces a configurable rate limit (default: <?= (int)API_THROTTLE_LIMIT ?> requests per <?= (int)API_THROTTLE_WINDOW ?>s per IP).
        Exceeding the limit returns <code>HTTP 429 Too Many Requests</code>. Adjust in <code>config.php</code>.</p>

        <div style="margin-top:30px; border-top:1px solid var(--border); padding-top:20px;">
            <a href="<?= htmlspecialchars(BASE_URL) ?>" class="back-btn">&larr; Back to Dashboard</a>
        </div>
    </div>
    ...
    <div class="console-section">
        ...
        <form id="apiForm">
            ...
            <div id="getWrapper" style="display:none;">
                <label>UUID</label>
                <input type="text" id="qrUuid" placeholder="e.g. 5f3a2b...">
            </div>
...
function generateKey() {
    ...
    document.getElementById('newKeyDisplay').innerText = hex;
    navigator.clipboard.writeText(hex).then(() => alert('New API Key copied to clipboard!'));
}
...
function showPayload(type) {
    ...
    document.getElementById('modalCode').textContent = JSON.stringify(payloads[type], null, 4);
...
document.getElementById('apiForm').addEventListener('submit', async function(e) {
    ...
    if (action === 'list') {
        ...
        fd.append('method', 'GET');
        fd.append('uuid', document.getElementById('qrUuid').value.trim());
    } else {
        // create
        fd.append('method', 'POST');
        const type = document.getElementById('qrType').value;
        let payload = { title: document.getElementById('qrTitle').value, type };
        ...
        else if (type === 'wifi')   { payload.ssid = document.getElementById('in-ssid').value; payload.pass = document.getElementById('in-pass').value; payload.enc = document.getElementById('in-enc').value; }
        else if (type === 'vcard')  { payload.fname = document.getElementById('in-fname').value; payload.lname = document.getElementById('in-lname').value; payload.phone = document.getElementById('in-vphone').value; payload.email = document.getElementById('in-vemail').value; }
        else if (type === 'sms')    { payload.phone = document.getElementById('in-sphone').value; payload.body = document.getElementById('in-sbody').value; }
        else if (type === 'email')  { payload.email = document.getElementById('in-eaddr').value; payload.subject = document.getElementById('in-esub').value; payload.body = document.getElementById('in-ebody').value; }

        fd.append('payload', JSON.stringify(payload));
    }

    try {
        const res  = await fetch('<?= htmlspecialchars($proxyUrl) ?>', { method: 'POST', body: fd });
